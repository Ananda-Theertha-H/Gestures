...
zoom_spread_buffer = 30  # more intentional for zoom in
zoom_cooldown = 2 # cooldown for zoom in/out
last_zoom_time = 0
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                if prev_y is not None and mode and now - last_action_time > cooldown:
                    ...
                    if abs(dy) > 30:
                        if mode == 'scroll':
                            pyautogui.scroll(-100 if dy > 0 else 100)
                            action_text = 'Scroll Down' if dy > 0 else 'Scroll Up'
                        elif mode == 'volume':
                            pyautogui.press(['volumedown'] * 7 if dy > 2 else ['volumeup'] * 5)
                            action_text = 'Volume Down' if dy > 2 else 'Volume Up'
